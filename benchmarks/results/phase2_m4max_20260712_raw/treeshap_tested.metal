...
// storage, 32/64/128/256-thread threadgroups, and rows_per_simdgroup in {1, 7, 1024};
...
inline void ShapFirstOrderImpl(device const float* X, device const PathElement* elements,
  ...
  for (ulong row = row_begin; row < row_end; row++) {
    ...
    OutputAccumulator<kSimdgroupAggregation>::Add(phis, p, row, e, is_root, phi, lane);
  }
}
...
// scheduling, threadgroup size, row-bank size, and row-tile size.
kernel void reduce_partials_serial(
  ...
  for (uint i = cell.begin; i < cell.end; ++i) sum += partials[partial_base + i];
