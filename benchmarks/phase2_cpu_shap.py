...
def normalize_shap_values(values: Any, rows: int, cols: int, groups: int):
    ...
    # The return TYPE disambiguates the layouts: legacy SHAP returns a per-class list
    # (handled above), while current shap.TreeExplainer documents multiclass ndarray
    # output as feature-last [samples, features, outputs]. Check feature-last FIRST so
    # the classes == features square shape — where both checks would match — follows
    # the documented contract instead of being misread as group-major.
