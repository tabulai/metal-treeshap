...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const size_t num_groups = std::strtoull(argv[3], nullptr, 10);
    const std::vector<double> intercepts =
        (argc >= 7) ? csv::ParseIntercepts(argv[6], num_groups)
                    : std::vector<double>(num_groups, 0.0);
    const uint64_t shuffle_seed = (argc >= 8) ? std::strtoull(argv[7], nullptr, 10) : 0;
    ...
    const size_t out_size = rows * num_groups * (cols + 1);
